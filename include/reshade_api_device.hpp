...
namespace reshade::api
{
	...
	inline uint32_t compute_crc32(uint32_t crc, const uint8_t *data, size_t size)
	{
	...
	inline uint32_t compute_crc32(const uint8_t *data, size_t size)
	{
		...
		return ~compute_crc32(crc, data, size);
	}
	// VUGGER_ADDON:
	...
	{
		struct device *get_device() { return _device; }

		// VUGGER_ADDON:
		virtual ~device_object() {}

		virtual const char *get_name() { return "TODO"; } // TODO: = 0;

		virtual void gui() {} // TODO: = 0;

		std::string		m_name;
		uint32_t		m_id	= 0;
		object_type		m_type	= object_type::unknown;
		struct device*	_device;	// TODO: Move to a ref

	protected:
		device_object(struct device* device, object_type type)
			: m_type(type), _device(device)
		{
		}

		void get_name(uint64_t device_object, const char *auto_name)
		{
			if (m_name.empty() && device_object)
			{
				// TODO: Push out to a virtual call into the api layer
				//uint32_t size = sizeof(const char *);
				//m_device.get_object_data(device_object, *reinterpret_cast<const uint8_t(*)[16]>(&WKPDID_D3DDebugObjectName), &size, nullptr);

				//if (size)
				//{
				//	m_name.resize(size + 1);
				//	m_device.get_object_data(device_object, *reinterpret_cast<const uint8_t(*)[16]>(&WKPDID_D3DDebugObjectName), &size, m_name.data());
				//}
				//else
				{
					m_name = auto_name;
...
namespace reshade::api
{
	...
	RESHADE_DEFINE_INTERFACE_WITH_BASE(device, api_object)
	{
		...
		virtual std::unique_ptr<sampler>  create_sampler(const sampler_desc &desc) = 0;	// VUGGER_ADDON:
		/// <summary>
		/// Instantly destroys a sampler that was previously created via <see cref="create_sampler"/>.
		/// </summary>
		virtual void destroy_sampler(sampler* ptr) = 0;		// VUGGER_ADDON
	...
	{
		command_list(device * device) : device_object(device, object_type::command_list) {}		// VUGGER_ADDON
		...
		virtual reshade::api::sampler *const *get_vs_samplers() const = 0;
		virtual reshade::api::sampler *const *get_hs_samplers() const = 0;
		virtual reshade::api::sampler *const *get_ds_samplers() const = 0;
		virtual reshade::api::sampler *const *get_gs_samplers() const = 0;
		virtual reshade::api::sampler *const *get_ps_samplers() const = 0;
		virtual reshade::api::sampler *const *get_cs_samplers() const = 0;
		// VUGGER_ADDON: END
	};
	...
	{
		command_queue(device * device) : device_object(device, object_type::command_queue) {}	// VUGGER_ADDON
	...
	{
		swapchain(device *device) : device_object(device, object_type::swapchain) {}	// VUGGER_ADDON
