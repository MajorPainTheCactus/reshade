...
//	{
//		if ((old_states[i] & reshade::api::resource_usage::shader_resource) != 0 && (new_states[i] & reshade::api::resource_usage::shader_resource) == 0 &&
//			// Ignore transitions to copy source or destination states, since those are not affected by the current SRV bindings
//			(new_states[i] & (reshade::api::resource_usage::depth_stencil | reshade::api::resource_usage::render_target)) != 0)
//			transitions_away_from_shader_resource_usage = true;
//		if ((old_states[i] & reshade::api::resource_usage::unordered_access) != 0 && (new_states[i] & reshade::api::resource_usage::unordered_access) == 0)
...
//}
//
//void reshade::d3d11::command_list_impl::begin_render_pass(uint32_t count, const reshade::api::render_pass_render_target_desc *rts, const reshade::api::render_pass_depth_stencil_desc *ds)
//{
//	assert(count <= D3D11_SIMULTANEOUS_RENDER_TARGET_COUNT);
//
//	temp_mem<reshade::api::resource_view, D3D11_SIMULTANEOUS_RENDER_TARGET_COUNT> rtv_handles(count);
//	for (uint32_t i = 0; i < count; ++i)
//	{
//		rtv_handles[i] = rts[i].view;
//
//		if (rts[i].load_op == reshade::api::render_pass_load_op::clear)
//			_orig->ClearRenderTargetView(reinterpret_cast<ID3D11RenderTargetView *>(rtv_handles[i].handle), rts[i].clear_color);
//	}
//
//	reshade::api::resource_view depth_stencil_handle = {};
//	if (ds != nullptr && ds->view.handle != 0)
//	{
//		depth_stencil_handle = ds->view;
//
//		if (const UINT clear_flags = (ds->depth_load_op == reshade::api::render_pass_load_op::clear ? D3D11_CLEAR_DEPTH : 0) | (ds->stencil_load_op == reshade::api::render_pass_load_op::clear ? D3D11_CLEAR_STENCIL : 0))
...
//}
//void reshade::d3d11::command_list_impl::end_render_pass()
//{
//	// Reset render targets
//	_orig->OMSetRenderTargets(0, nullptr, nullptr);
//}
//void reshade::d3d11::command_list_impl::bind_render_targets_and_depth_stencil(uint32_t count, const reshade::api::resource_view *rtvs, reshade::api::resource_view dsv)
//{
//	assert(count <= D3D11_SIMULTANEOUS_RENDER_TARGET_COUNT);
//
//#ifndef _WIN64
...
//}
//
//void reshade::d3d11::command_list_impl::bind_pipeline(reshade::api::pipeline_stage stages, reshade::api::pipeline pipeline)
//{
...
//		assert((stages & reshade::api::pipeline_stage::all_graphics) != 0);
...
//	case reshade::api::pipeline_stage::geometry_shader | api::pipeline_stage::stream_output:
...
//	}
//}
//void reshade::d3d11::command_list_impl::bind_pipeline_states(uint32_t count, const reshade::api::dynamic_state *states, const uint32_t *values)
//{
//	for (uint32_t i = 0; i < count; ++i)
//	{
//		switch (states[i])
...
//			_orig->IASetPrimitiveTopology(convert_primitive_topology(static_cast<reshade::api::primitive_topology>(values[i])));
//			break;
//		default:
//			assert(false);
//			break;
...
//	}
//}
//void reshade::d3d11::command_list_impl::bind_viewports(uint32_t first, uint32_t count, const reshade::api::viewport *viewports)
//{
//	if (first != 0)
//		return;
//
...
//}
//void reshade::d3d11::command_list_impl::bind_scissor_rects(uint32_t first, uint32_t count, const reshade::api::rect *rects)
//{
//	if (first != 0)
//		return;
//
...
//}
//
//void reshade::d3d11::command_list_impl::bind_samplers(reshade::api::shader_stage stages, uint32_t first, uint32_t count, const reshade::api::sampler *samplers)
//{
...
//}
//void reshade::d3d11::command_list_impl::bind_shader_resource_views(reshade::api::shader_stage stages, uint32_t first, uint32_t count, const reshade::api::resource_view *views)
//{
...
//}
//void reshade::d3d11::command_list_impl::bind_unordered_access_views(reshade::api::shader_stage stages, uint32_t first, uint32_t count, const reshade::api::resource_view *views)
//{
...
//}
//void reshade::d3d11::command_list_impl::bind_constant_buffers(reshade::api::shader_stage stages, uint32_t first, uint32_t count, const reshade::api::buffer_range *buffer_ranges)
//{
...
//}
//
//void reshade::d3d11::command_list_impl::push_constants(reshade::api::shader_stage stages, reshade::api::pipeline_layout layout, uint32_t layout_param, uint32_t first, uint32_t count, const void *values)
//{
//	assert(first == 0);
//
//	if (count == 0)
...
//	{
//		const reshade::api::descriptor_range &range = reinterpret_cast<pipeline_layout_impl *>(layout.handle)->ranges[layout_param];
//
...
//}
//void reshade::d3d11::command_list_impl::push_descriptors(reshade::api::shader_stage stages, reshade::api::pipeline_layout layout, uint32_t layout_param, const reshade::api::descriptor_set_update &update)
//{
...
//		bind_samplers(stages, first, update.count, static_cast<const reshade::api::sampler *>(update.descriptors));
//		break;
//	case reshade::api::descriptor_type::shader_resource_view:
//		bind_shader_resource_views(stages, first, update.count, static_cast<const reshade::api::resource_view *>(update.descriptors));
//		break;
//	case reshade::api::descriptor_type::unordered_access_view:
//		bind_unordered_access_views(stages, first, update.count, static_cast<const reshade::api::resource_view *>(update.descriptors));
//		break;
//	case reshade::api::descriptor_type::constant_buffer:
//		bind_constant_buffers(stages, first, update.count, static_cast<const reshade::api::buffer_range *>(update.descriptors));
//		break;
//	default:
//		assert(false);
//		break;
//	}
//}
//void reshade::d3d11::command_list_impl::bind_descriptor_sets(reshade::api::shader_stage stages, reshade::api::pipeline_layout layout, uint32_t first, uint32_t count, const reshade::api::descriptor_set *sets)
//{
//	for (uint32_t i = 0; i < count; ++i)
//	{
...
//			layout,
//			first + i,
//			reshade::api::descriptor_set_update{ {}, 0, 0, set_impl->count, set_impl->type, set_impl->descriptors.data() });
//	}
//}
//
//void reshade::d3d11::command_list_impl::bind_index_buffer(reshade::api::resource buffer, uint64_t offset, uint32_t index_size)
//{
...
//}
//void reshade::d3d11::command_list_impl::bind_vertex_buffers(uint32_t first, uint32_t count, const reshade::api::resource *buffers, const uint64_t *offsets, const uint32_t *strides)
//{
...
//}
//void reshade::d3d11::command_list_impl::bind_stream_output_buffers(uint32_t first, uint32_t count, const reshade::api::resource *buffers, const uint64_t *offsets, const uint64_t *)
//{
...
//}
//
//void reshade::d3d11::command_list_impl::draw(uint32_t vertex_count, uint32_t instance_count, uint32_t first_vertex, uint32_t first_instance)
//{
//	_orig->DrawInstanced(vertex_count, instance_count, first_vertex, first_instance);
//}
//void reshade::d3d11::command_list_impl::draw_indexed(uint32_t index_count, uint32_t instance_count, uint32_t first_index, int32_t vertex_offset, uint32_t first_instance)
//{
//	_orig->DrawIndexedInstanced(index_count, instance_count, first_index, vertex_offset, first_instance);
//}
//void reshade::d3d11::command_list_impl::dispatch(uint32_t group_count_x, uint32_t group_count_y, uint32_t group_count_z)
//{
//	_orig->Dispatch(group_count_x, group_count_y, group_count_z);
//}
//void reshade::d3d11::command_list_impl::draw_or_dispatch_indirect(reshade::api::indirect_command type, reshade::api::resource buffer, uint64_t offset, uint32_t draw_count, uint32_t stride)
//{
//	assert(offset <= std::numeric_limits<UINT>::max());
//
//	switch (type)
...
//}
//
//void reshade::d3d11::command_list_impl::copy_resource(reshade::api::resource src, reshade::api::resource dst)
//{
//	assert(src.handle != 0 && dst.handle != 0);
//
//	_orig->CopyResource(reinterpret_cast<ID3D11Resource *>(dst.handle), reinterpret_cast<ID3D11Resource *>(src.handle));
//}
//void reshade::d3d11::device_context_impl::copy_buffer_region(reshade::api::resource src, uint64_t src_offset, reshade::api::resource dst, uint64_t dst_offset, uint64_t size)
//{
//	assert(src.handle != 0 && dst.handle != 0);
//
//	if (size == UINT64_MAX)
...
//}
//void reshade::d3d11::command_list_impl::copy_buffer_region(reshade::api::resource src, uint64_t src_offset, reshade::api::resource dst, uint64_t dst_offset, uint64_t size)
//{
//	assert(src.handle != 0 && dst.handle != 0);
//
//	if (size == UINT64_MAX)
...
//		reinterpret_cast<ID3D11Resource *>(src.handle), 0, &src_box);
//}
//void reshade::d3d11::command_list_impl::copy_buffer_to_texture(reshade::api::resource, uint64_t, uint32_t, uint32_t, reshade::api::resource, uint32_t, const reshade::api::subresource_box *)
//{
//	assert(false);
//}
//void reshade::d3d11::command_list_impl::copy_texture_region(reshade::api::resource src, uint32_t src_subresource, const reshade::api::subresource_box *src_box, reshade::api::resource dst, uint32_t dst_subresource, const api::subresource_box *dst_box, api::filter_mode)
//{
//	assert(src.handle != 0 && dst.handle != 0);
...
//}
//void reshade::d3d11::command_list_impl::copy_texture_to_buffer(reshade::api::resource, uint32_t, const reshade::api::subresource_box *, reshade::api::resource, uint64_t, uint32_t, uint32_t)
//{
//	assert(false);
//}
//void reshade::d3d11::command_list_impl::resolve_texture_region(reshade::api::resource src, uint32_t src_subresource, const reshade::api::subresource_box *src_box, reshade::api::resource dst, uint32_t dst_subresource, int32_t dst_x, int32_t dst_y, int32_t dst_z, api::format format)
//{
//	assert(src.handle != 0 && dst.handle != 0);
...
//}
//
//void reshade::d3d11::command_list_impl::clear_depth_stencil_view(reshade::api::resource_view dsv, const float *depth, const uint8_t *stencil, uint32_t rect_count, const reshade::api::rect *)
//{
...
//}
//void reshade::d3d11::command_list_impl::clear_render_target_view(reshade::api::resource_view rtv, const float color[4], uint32_t rect_count, const reshade::api::rect *)
//{
...
//}
//void reshade::d3d11::command_list_impl::clear_unordered_access_view_uint(reshade::api::resource_view uav, const uint32_t values[4], uint32_t rect_count, const reshade::api::rect *)
//{
...
//}
//void reshade::d3d11::command_list_impl::clear_unordered_access_view_float(reshade::api::resource_view uav, const float values[4], uint32_t rect_count, const reshade::api::rect *)
//{
//	assert(uav.handle != 0 && rect_count == 0); // Clearing rectangles is not supported
//
...
//}
//
//void reshade::d3d11::command_list_impl::generate_mipmaps(reshade::api::resource_view srv)
//{
//	assert(srv.handle != 0);
//
//	_orig->GenerateMips(reinterpret_cast<ID3D11ShaderResourceView *>(srv.handle));
//}
//void reshade::d3d11::command_list_impl::clear_state()
//{
//	_orig->ClearState();
//}
//
//void reshade::d3d11::command_list_impl::begin_query(reshade::api::query_pool pool, reshade::api::query_type, uint32_t index)
//{
//	assert(pool.handle != 0);
//
//	_orig->Begin(reinterpret_cast<query_pool_impl *>(pool.handle)->queries[index].get());
//}
//void reshade::d3d11::command_list_impl::end_query(reshade::api::query_pool pool, reshade::api::query_type, uint32_t index)
//{
//	assert(pool.handle != 0);
//
//	_orig->End(reinterpret_cast<query_pool_impl *>(pool.handle)->queries[index].get());
//}
//void reshade::d3d11::command_list_impl::copy_query_pool_results(reshade::api::query_pool, reshade::api::query_type, uint32_t, uint32_t, reshade::api::resource, uint64_t, uint32_t)
//{
//	assert(false);
//}
//
//void reshade::d3d11::command_list_impl::begin_debug_event(const char *label, const float[4])
//{
...
//}
//void reshade::d3d11::command_list_impl::end_debug_event()
//{
//	if (_annotations == nullptr)
//		return;
//
...
//}
//void reshade::d3d11::command_list_impl::insert_debug_marker(const char *label, const float[4])
//{
//	assert(label != nullptr);
//
//	if (_annotations == nullptr)
...
// VUGGER_ADDON: BEGIN
void reshade::d3d11::device_context_impl::execute_command_lists(uint32_t count, api::command_list** command_lists, bool restore_state)
{
	assert(count == 1);
	assert(restore_state == false);

	_orig->ExecuteCommandList(reinterpret_cast<ID3D11CommandList*>(command_lists[0]->get_native()), restore_state);
}
// VUGGER_ADDON: END
